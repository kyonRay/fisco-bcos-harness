---
name: fbh-gate
description: milestone 门禁：在负责人本机跑完整集成测试+SIT（包装 fisco-bcos-release-gate），通过则回写表格勾选完成，失败则自动建缺陷行并企微@归因作者。milestone 到点验收、发版前验证时使用。
---

# /fbh-gate — milestone 门禁

只有 milestone 负责人跑（ADR-0004）。门禁命令本体是 fisco-bcos-release-gate
（或团队配置的其它命令），fbh 只包装：执行 → 按结果回写表格 + 企微通知。
